...
module.exports.run = async (bot, message, args,sql) => {
   ...
   if(!maincat) return message.channel.send("There is no category called : `"+args.join(" ")+"`")
    ...
    if(!rows){
        sql.prepare(`INSERT INTO guilds (guildID, chname, catname) VALUES ('${message.author.guild.id}', 'إنشاء روم','${args.join(" ")}')`).run();
    }else{
        sql.prepare(`UPDATE guilds SET catname = '${args.join(" ")}' WHERE guildID = '${message.author.guild.id}'`).run();
    }
