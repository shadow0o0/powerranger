...
module.exports.run = async (bot, message, args,sql) => {
   ...
   if(!maincat) return message.channel.send("There is no voice channel called : `"+args.join(" ")+"`")
    ...
    if(!rows){
        sql.prepare(`INSERT INTO guilds (guildID, chname, catname) VALUES ('${message.guild.id}', '${args.join(" ")}','الرومات المؤقته')`).run();
    }else{
        sql.prepare(`UPDATE guilds SET chname = '${args.join(" ")}' WHERE guildID = '${message.guild.id}'`).run();
    }
